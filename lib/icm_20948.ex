...
#    Logger.debug(
 #     "accel mpss: #{ViaUtils.Format.eftb_list([accel_x_mpss, accel_y_mpss, accel_z_mpss], 3)}"
  ...
  def set_raw_data_ready_interrupt(icm, enable_interrupt) do
    {icm, int_enable_1_value} =
      get_new_register_value(
        icm,
        Reg.IntEnable1,
        %{Keys.raw_data_0_rdy_en() => bool_to_int(enable_interrupt)},
        Reg.agb0_reg_int_enable_1(),
        0
      )

    IcmDevice.write(icm, Reg.agb0_reg_int_enable_1(), <<int_enable_1_value>>)
    icm
  end
